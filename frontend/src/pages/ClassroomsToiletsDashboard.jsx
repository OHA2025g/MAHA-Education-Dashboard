...
import { Building2, RefreshCw, Upload, School, BarChart3, Droplets, Bath, AlertTriangle, CheckCircle2, XCircle, Heart, Users, Wrench, ShieldCheck, TrendingUp } from "lucide-react";
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [overviewRes, blockRes, conditionRes, toiletRes, hygieneRes, riskRes, constructionRes, equityRes, rankRes] = await Promise.all([
        axios.get(`${API}/classrooms-toilets/overview`),
        axios.get(`${API}/classrooms-toilets/block-wise`),
        axios.get(`${API}/classrooms-toilets/classroom-condition`),
        axios.get(`${API}/classrooms-toilets/toilet-distribution`),
        axios.get(`${API}/classrooms-toilets/hygiene-metrics`),
        axios.get(`${API}/classrooms-toilets/risk-schools`),
        axios.get(`${API}/classrooms-toilets/construction-status`),
        axios.get(`${API}/classrooms-toilets/equity-metrics`),
        axios.get(`${API}/classrooms-toilets/top-bottom-blocks`)
      ]);
      setOverview(overviewRes.data);
      setBlockData(blockRes.data);
      setClassroomCondition(conditionRes.data);
      setToiletDistribution(toiletRes.data);
      setHygieneMetrics(hygieneRes.data);
      setRiskSchools(riskRes.data);
      setConstructionStatus(constructionRes.data);
      setEquityMetrics(equityRes.data);
      setTopBottomBlocks(rankRes.data);
    } catch (error) {
