...
async def get_age_enrolment_overview():
    ...
    # Get unique schools and blocks
    schools = await db.age_enrolment.distinct("udise_code")
    blocks = await db.age_enrolment.distinct("block_name")
...
async def get_top_schools_by_enrolment(n: int = Query(20, description="Number of schools")):
...
async def get_age_enrolment_data_quality():
    ...
    # Check for zero enrolment
    zero_cursor = db.age_enrolment.find({"total_students": 0})
...
async def import_age_enrolment(
    ...
    try:
        ...
        logger.error(f"Age-wise Enrolment import error: {str(e)}")
...
async def process_age_enrolment_file(file_path: str, filename: str, import_id: str):
    ...
    try:
        logger.info(f"Processing Age-wise Enrolment file: {filename}")
        
        df = pd.read_excel(file_path)
        logger.info(f"Age-wise Enrolment file loaded: {len(df)} rows, {len(df.columns)} columns")
        ...
        for idx, row in df.iterrows():
            try:
                ...
                logger.error(f"Error processing age enrolment row {idx}: {str(e)}")
                continue
        
        logger.info(f"Age-wise Enrolment import completed: {records_processed} records")
        
    except Exception as e:
        logger.error(f"Age-wise Enrolment import failed: {str(e)}")
