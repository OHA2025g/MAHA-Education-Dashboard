...
async def get_ctteacher_overview():
    ...
    pipeline = [
        {
            "$group": {
                ...
                "aadhaar_verified": {"$sum": "$aadhaar_verified"},
                "completed": {"$sum": {"$cond": [{"$eq": ["$completion_status", "Completed"]}, 1, 0]}},
                "ctet_qualified": {"$sum": {"$cond": [{"$gt": ["$ctet_qualified", 0]}, 1, 0]}},
                "nishtha_completed": {"$sum": {"$cond": [{"$gt": ["$training_nishtha", 0]}, 1, 0]}},
            }
        }
    ]
    ...
    # Get unique counts
    unique_teachers = await db.ctteacher_analytics.distinct("teacher_code")
    unique_schools = await db.ctteacher_analytics.distinct("udise_code")
    blocks = await db.ctteacher_analytics.distinct("block_name")
...
async def get_ctteacher_data_quality():
    ...
    total_count = await db.ctteacher_analytics.count_documents({})
    
    # Missing staff code
    missing_staff = await db.ctteacher_analytics.count_documents({"teacher_code": {"$in": [None, "", "nan"]}})
    
    # Aadhaar not verified (aadhaar_verified is 1 for verified, 2 or other for not)
    aadhaar_not_verified = await db.ctteacher_analytics.count_documents({"aadhaar_verified": {"$ne": 1}})
    
    # Not completed
    not_completed = await db.ctteacher_analytics.count_documents({"completion_status": {"$ne": "Completed"}})
...
async def import_ctteacher_data(
    ...
    try:
        ...
        logger.error(f"CTTeacher import error: {str(e)}")
...
async def process_ctteacher_file(file_path: str, filename: str, import_id: str):
    ...
    try:
        logger.info(f"Processing CTTeacher file: {filename}")
        
        df = pd.read_excel(file_path)
        logger.info(f"CTTeacher file loaded: {len(df)} rows, {len(df.columns)} columns")
        ...
        for idx, row in df.iterrows():
            try:
                ...
                logger.error(f"Error processing ctteacher row {idx}: {str(e)}")
                continue
        
        logger.info(f"CTTeacher import completed: {records_processed} records")
        
    except Exception as e:
        logger.error(f"CTTeacher import failed: {str(e)}")
