...
async def get_risk_schools():
    ...
    zero_toilet_cursor = db.classrooms_toilets.find(
        {"$or": [{"boys_toilets_total": 0}, {"girls_toilets_total": 0}]},
    ...
    # Risk counts
    total_schools = await db.classrooms_toilets.count_documents({})
    zero_toilet_count = await db.classrooms_toilets.count_documents(
        {"$or": [{"boys_toilets_total": 0}, {"girls_toilets_total": 0}]}
    )
    no_handwash_count = await db.classrooms_toilets.count_documents({"handwash_facility": False})
...
async def import_classrooms_toilets(
    ...
    try:
        ...
        logger.error(f"Classrooms Toilets import error: {str(e)}")
...
async def process_classrooms_toilets_file(file_path: str, filename: str, import_id: str):
    ...
    try:
        logger.info(f"Processing Classrooms & Toilets file: {filename}")
        ...
        logger.info(f"Classrooms & Toilets import complete: {records_processed} records")
        
    except Exception as e:
        logger.error(f"Error processing Classrooms & Toilets file: {str(e)}")
