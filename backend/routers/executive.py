...
async def get_student_identity_compliance(
    ...
    return {
        ...
        "block_performance": [{"block_name": b["_id"], "aadhaar_pct": round(b["aadhaar_pct"], 1), "name_mismatch_pct": round(b["name_mismatch_pct"], 2)} for b in block_data]
    }
...
async def get_infrastructure_facilities(
    ...
    return {
        ...
        "block_performance": [{"block_name": b["_id"], "classroom_health": round(b["classroom_health"], 1), "toilet_pct": round(b["toilet_functional_pct"], 1)} for b in block_data]
    }
...
async def get_teacher_staffing(
    ...
    ct_pipeline = prepend_match([
        {"$group": {
            "_id": None,
            "total_records": {"$sum": 1},
            "unique_teachers": {"$addToSet": "$teacher_id"},
    ...
    total_teachers = ct.get("total_records", 0)
    unique_teachers = len(ct.get("unique_teachers", [])) if ct.get("unique_teachers") else 0
    total_schools = len(ct.get("total_schools", [])) if ct.get("total_schools") else 0
    
    # Calculate KPIs
    aadhaar_verified_pct = round(ct.get("aadhaar_verified", 0) / total_teachers * 100, 1) if total_teachers > 0 else 0
    ctet_pct = round(ct.get("ctet_qualified", 0) / total_teachers * 100, 1) if total_teachers > 0 else 0
    nishtha_pct = round(ct.get("nishtha_completed", 0) / total_teachers * 100, 1) if total_teachers > 0 else 0
    female_pct = round(ct.get("female_count", 0) / total_teachers * 100, 1) if total_teachers > 0 else 0
    ...
    return {
        "summary": {
            ...
            "total_schools": total_schools,
            "avg_per_school": round(total_teachers / total_schools, 1) if total_schools > 0 else 0,
        ...
        "block_performance": [{"block_name": b["_id"], "teachers": b["teachers"], "ctet_pct": round(b["ctet_pct"], 1), "nishtha_pct": round(b["nishtha_pct"], 1)} for b in block_data]
    }
...
async def get_operational_performance(
    ...
    return {
        ...
        "block_performance": [{"block_name": b["_id"], "completion_rate": round(b["completion_rate"], 2), "certification_rate": round(b["certification_rate"], 1)} for b in block_data]
    }
...
async def get_school_health_index(
    ...
    for i, block in enumerate(infrastructure.get("block_performance", [])[:20]):
        ...
        identity_block = next((b for b in identity.get("block_performance", []) if b["block_name"] == block_name), {})
        teacher_block = next((b for b in teacher.get("block_performance", []) if b["block_name"] == block_name), {})
        operational_block = next((b for b in operational.get("block_performance", []) if b["block_name"] == block_name), {})
