...
async def process_infrastructure_file(file_path: str, filename: str, import_id: str):
    ...
    try:
        logger.info(f"Processing Infrastructure file: {filename}")
        
        df = pd.read_excel(file_path, engine='openpyxl')
        df.columns = [str(col).strip().lower().replace(' ', '_').replace('/', '_').replace('&', 'and') for col in df.columns]
        
        logger.info(f"Infrastructure file columns: {list(df.columns)}")
        ...
        logger.info(f"Infrastructure import completed: {records_processed} records")
