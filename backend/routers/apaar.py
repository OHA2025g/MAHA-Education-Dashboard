...
async def get_apaar_overview():
    ...
    # Get block count
    blocks = await db.apaar_analytics.distinct("block_name", {})
...
async def get_top_pending_schools(n: int = Query(20, description="Number of schools")):
    """Get top schools by pending APAAR count"""
    pipeline = [
        {"$match": {"total_student": {"$gt": 0}}},
        {
            "$addFields": {
...
async def get_low_performing_schools(threshold: float = Query(80, description="Generation rate threshold")):
    """Get schools below generation rate threshold"""
    pipeline = [
        {"$match": {"total_student": {"$gt": 50}}},
        {
            "$addFields": {
                "generation_rate": {
                    "$multiply": [{"$divide": ["$total_generated", "$total_student"]}, 100]
...
async def import_apaar_status(
    ...
    try:
        ...
        logger.error(f"APAAR import error: {str(e)}")
...
async def process_apaar_file(file_path: str, filename: str, import_id: str):
    ...
    try:
        logger.info(f"Processing APAAR file: {filename}")
        
        df = pd.read_excel(file_path)
        logger.info(f"APAAR file loaded: {len(df)} rows, {len(df.columns)} columns")
        ...
        for idx, row in df.iterrows():
            try:
                ...
                logger.error(f"Error processing APAAR row {idx}: {str(e)}")
                continue
        
        logger.info(f"APAAR import completed: {records_processed} records")
        
    except Exception as e:
        logger.error(f"APAAR import failed: {str(e)}")
